#自适应权重
# Best checkpoint: models/fakett/weights/best_model_fake_TT_bst_0.8462.pth
# 测试结果:
# Test Accuracy: 0.8462
# Weighted Precision: 0.8491
# Weighted Recall: 0.8462
# Weighted F1: 0.8472
# Fake - Precision: 0.7524, Recall: 0.7980, F1: 0.7745
# Real - Precision: 0.8969, Recall: 0.8700, F1: 0.8832
# 训练完成!
...
class MultiModalNewsDataset(Dataset):
    ...
    def _load_pkl(self, path):
        ...
        # 新增：处理纯列表格式，其中每个元素是一个dict {'video_id': ..., 'features': ...}
        if isinstance(raw, list) and len(raw) > 0 and isinstance(raw[0], dict) and 'video_id' in raw[0]:
            print(f"列表字典格式，共{len(raw)}条")
            return {item['video_id']: np.array(item['features'], dtype=np.float32) for item in raw}
...
class CoLaVID(nn.Module):
    ...
    def __init__(self, 
                 ...
                 kernel_size=5,     # 对应原本的 kernel_size=3
...
def train_model(model, train_loader, val_loader, test_loader, 
                ...
                model_params=None, csv_file="model_results.csv"):
    """
    完整的模型训练和评估流程
    ...
    Args:
        ...
        learning_rate: 学习率
        device: 设备
        model_params: 模型参数字典，包含dropout, fixed_dim, conv_out, kernel_size, cls_hidden, cls_dropout等
        csv_file: CSV文件保存路径
    """
    # 修改损失函数初始化
    class_weights = torch.tensor([1.0, 1.4], device=device)  # 调整类别权重
...
# 在 train_model 函数中
    ...
    try:
        ...
        cmd = [
            ...
            "--dropout", str(model_params.get('dropout', 0.4)),
            "--conv_out", str(model_params.get('conv_out', 128)),
            "--kernel_size", str(model_params.get('kernel_size', 5)),
        ...
        # 准备保存到CSV的结果字典
        results_dict = {
            'Drop': model_params.get('dropout', 0.4),
...
BASE = os.environ.get(
    "EGRT_FAKETT_DATA_DIR",
    os.path.join(REPO_ROOT, "reproduction_data", "FakeTT"),
)
SPLIT_DIR = os.path.join(BASE, "data_split")
FEATURE_DIR = os.environ.get(
    "EGRT_FAKETT_FEATURE_DIR",
...
feature_info = {
    'audio_hubert': {
        'type': 'pkl',
        'path': os.path.join(FEATURE_DIR, "audio_hubert_feats_av_filled.pkl")
    },
    'audio_vggish': {
        'type': 'pkl',
        'path': os.path.join(FEATURE_DIR, "audio_vggish_filled.pkl")
    },
    'text_bert': {
        'type': 'pkl',
        'path': os.path.join(FEATURE_DIR, "title_ocr_bert.pkl")
    },
    'text_xclip': {
        'type': 'pkl',
    ...
    'internet_bert': {
        'type': 'pkl',
        'path': os.path.join(FEATURE_DIR, "internet_bert.pkl")
    },

    'internet_xclip': {
        'type': 'pkl',
        'path': os.path.join(FEATURE_DIR, "internet_xclip.pkl")
    },  
    'internet_claim_bert': {
        'type': 'pkl',
        'path': os.path.join(FEATURE_DIR, "internet_claim_bert.pkl")
    },

    'internet_claim_xclip': {
        'type': 'pkl',
        'path': os.path.join(FEATURE_DIR, "internet_claim_xclip.pkl")
    },  
...
if __name__ == "__main__":
    ...
    model_params = {
        ...
        'nhead': 8,
        'dropout': 0.3,        # Drop
        'conv_out': 128,       # Conv_Out
        'kernel_size': 3,      # Kernel_Size
        'cls_hidden': 64,      # Cls_Hidden
        'cls_dropout': 0.4     # Cls_Drop
    }
